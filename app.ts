...
function buildSvgPlot(levels: number[], values: number[], cssClass: string, label: string) {
...
function buildDeformedWallSvg(levels: number[], displacements: number[]) {
  ...
  const minLevel = Math.min(...levels);
  const maxLevel = Math.max(...levels);
  const maxAbsDisp = Math.max(1e-6, ...displacements.map((value: number) => Math.abs(value)));
  const path = displacements.map((value: number, index: number) => {
...
export async function runAnalysis(project: ProjectInput, fetchImpl: typeof fetch = fetch) {
...
function updateProjectPhaseSelect(project: ProjectInput, select: HTMLSelectElement, phaseIndex: number) {
...
function updateResultPhaseSelect(result: any, select: HTMLSelectElement, phaseIndex: number) {
